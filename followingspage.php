    ...
    <section id = "sectionfeed">
      <?php
        ...
        try {
          ...
          $sql = "SELECT `follower_table`.*,`user`.`Account_Name`,`user`.`Profile_Picture` FROM `follower_table` LEFT JOIN `user` ON `follower_table`.`Followed_User_ID`=`user`.`User_ID` WHERE `follower_table`.`Following_User_ID`=?";
          ...
          while($row = $stmt->fetch())
          {
            ...
            echo "<form action = 'clickaccount.php' method = 'POST'>
                  ...
                  <form action = 'deletefollowing.php' method = 'POST'>
                  <div>
                    <button id = 'buttondelete' name = 'deleteacc' type = 'submit' value = '$userid'><h4 id ='profilesearch'>Unfollow</h4></button>
                  </div>
                  </form>
                  ";
          }
